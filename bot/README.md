...
`.github/workflows/bot.yml` (im Repo-Root) definiert zwei Jobs und triggert nur
bei Änderungen unter `bot/`:
...
```bash
cd /opt/sonance/bot && docker compose pull && docker compose up -d
```
